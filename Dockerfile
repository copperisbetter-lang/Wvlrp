...
CMD ["/bin/sh", "-c", "python3 /viewer_count.py & EAST_SOURCE=\"${MTX_PATHS_EASTBANK_SOURCE:-$WVLRP_RTSP_SOURCE}\"; ROOST_SOURCE=\"${MTX_PATHS_ROOST_SOURCE:-$WVLRP_ROOST_RTSP_SOURCE}\"; env -u MTX_PATHS_EASTBANK_SOURCE -u MTX_PATHS_ROOST_SOURCE /mediamtx /mediamtx.yml & sleep 2; if [ -n \"$EAST_SOURCE\" ]; then (while true; do ffmpeg -hide_banner -loglevel warning -rtsp_transport tcp -i \"$EAST_SOURCE\" -map 0:v:0 -an -c:v copy -f rtsp -rtsp_transport tcp rtsp://127.0.0.1:8554/eastbank; rc=$?; echo \"[eastbank] ffmpeg exited rc=$rc; retrying in 10s\" >&2; sleep 10; done) & fi; if [ -n \"$ROOST_SOURCE\" ]; then (while true; do ffmpeg -hide_banner -loglevel warning -fflags +genpts+discardcorrupt -use_wallclock_as_timestamps 1 -rtsp_transport tcp -i \"$ROOST_SOURCE\" -map 0:v:0 -map 0:a:0? -c:v copy -c:a aac -profile:a aac_low -b:a 64k -ar 48000 -ac 1 -max_muxing_queue_size 4096 -max_interleave_delta 1000000 -muxdelay 0.1 -muxpreload 0.1 -f rtsp -rtsp_transport tcp rtsp://127.0.0.1:8554/roost; rc=$?; echo \"[roost] ffmpeg exited rc=$rc; retrying in 10s\" >&2; sleep 10; done) & fi; exec nginx -g 'daemon off;'"]
