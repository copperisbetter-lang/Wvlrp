...
from ptz_roost import move, configured

DATABASE_URL = os.environ.get("DATABASE_URL", "")
SECRET_KEY = os.environ.get("CHAT_SECRET_KEY", "change-me").encode()

app = FastAPI(title="WVLRP Roost PTZ Relay")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://wvlrp.com", "https://www.wvlrp.com", "https://copperisbetter-lang.github.io"],
...
def current_user(authorization: Optional[str]) -> dict:
    ...
    with psycopg.connect(DATABASE_URL, row_factory=dict_row) as conn, conn.cursor() as cur:
        cur.execute("SELECT id,role,support_level,support_amount_cents,disabled FROM chat_users WHERE id=%s", (payload["uid"],))
        user = cur.fetchone()
        cur.execute("SELECT value FROM site_settings WHERE key='ptz_sponsor_min_cents'")
        row = cur.fetchone()
    if not user or user["disabled"]: raise HTTPException(401, "Account unavailable")
    threshold = None
    if row and row["value"] not in (None, ""):
        try: threshold = max(0, int(row["value"]))
        except Exception: threshold = None
    is_admin = user["role"] in {"admin", "super"}
    allowed = is_admin or (user["support_level"] == "sponsor" and threshold is not None and int(user["support_amount_cents"] or 0) >= threshold)
    if not allowed: raise HTTPException(403, "PTZ access required")
    return user
