...
.hatch-panel{position:relative;z-index:20;width:min(94%,680px);margin:10px auto 16px;max-height:none;display:none;flex-direction:column;border-radius:20px;overflow:hidden;background:#06130e;border:1px solid #ffffff3b;box-shadow:0 22px 58px #000d;color:#fff;font-family:Arial,sans-serif}
...
@media(max-width:620px){.hatch-launcher{right:12px;bottom:14px;width:60px;height:60px}.hatch-panel{width:calc(100% - 20px);margin:10px auto 14px;max-height:none}}
`;
...
anchor.insertAdjacentElement('afterend',tools);tools.insertAdjacentElement('afterend',panel);
